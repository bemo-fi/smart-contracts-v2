...
export async function run(provider: NetworkProvider) {
    ...
    const sendCommissionMultisigPayload = getSendCommissionPayload()
    const sendCommissionMultisigMsg = getAdminMultisigInternalPayload(sendCommissionMultisigPayload, 0, 28800)

    await admin.sendInternal(provider.sender(), toNano("0.03"), sendCommissionMultisigMsg)
}
