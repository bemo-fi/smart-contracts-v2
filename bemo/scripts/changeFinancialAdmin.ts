...
export async function run(provider: NetworkProvider) {
    ...
    const changeAdminMultisigPayload = getChangeAdminMultisigPayload("New Admin Multisig Address");
    const changeAdminMultisigMsg = getAdminMultisigInternalPayload(changeAdminMultisigPayload, 0, 28800);

    await admin.sendInternal(provider.sender(), toNano("0.03"), changeAdminMultisigMsg);
}
