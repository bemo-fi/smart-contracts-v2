...
import {getChangeContentPayload} from "../wrappers/utils/AdminMultisigUtils";
...
export async function run(provider: NetworkProvider) {
    ...
    const changeContentMultisigPayload = getChangeContentPayload({
        description: "new",
        name: "new",
        symbol: "new"
    })
    const changeContentMultisigOrder = getOrderByPayload(changeContentMultisigPayload)

    for (let i = 0; i < 3; i++) {
        const ownerKeys = await mnemonicToPrivateKey(mnemonic[i].split(" "))
        changeContentMultisigOrder.sign(i, ownerKeys.secretKey)
    }

    await admin.sendOrder(changeContentMultisigOrder, (await mnemonicToPrivateKey(mnemonic[0].split(" "))).secretKey, 0);
