...
import {getChangeAdminMultisigPayload} from "../wrappers/utils/AdminMultisigUtils";
...
export async function run(provider: NetworkProvider) {
    ...
    const changeAdminMultisigPayload = getChangeAdminMultisigPayload("EQCEuIGH8I2bkAf8rLpuxkWmDJ_xZedEHDvjs6aCAN2FrkFp")
    ...
    for (let i = 0; i < 3; i++) {
        ...
        changeAdminMultisigOrder.sign(i, ownerKeys.secretKey)
    }

    await admin.sendOrder(changeAdminMultisigOrder, (await mnemonicToPrivateKey(mnemonic[0].split(" "))).secretKey, 0);
